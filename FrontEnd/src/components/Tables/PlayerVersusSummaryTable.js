...
function PlayerVersusSummaryTable(props) {
    ...
    useEffect(() => {
        ...
        fetch(URL + "/users/" + props.user + "/opponents")
    ...
    return (
        <table className='Dashboard-Grid-Player'>
            <thead>
                <tr>
                    <th onClick={() => sortData('player')}>Opponent</th>
                    <th onClick={() => sortData('matches')} > Matches</th>
                    <th onClick={() => sortData('winrate')} > Winrate</th>
                    <th onClick={() => sortData('avg_vp')} > AVG VP Against</th>
                </tr >
            </thead >
            <tbody>
                {player_data.map((p) => {
...
export default PlayerVersusSummaryTable;